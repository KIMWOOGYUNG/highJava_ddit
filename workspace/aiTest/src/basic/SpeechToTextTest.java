...
/*
	IBM Watson Speech to Text 서비스는 
	IBM의 음성 인식 기능을 응용 프로그램에 추가할 수 있는 API를 제공한다.
	이 서비스는 다양한 언어 및 오디오 형식의 음성을 빠르게 문자로 변환한다.
	모든 응답 내용은 UTF-8 인코딩의 JSON형식으로 반환한다. 
...
public class SpeechToTextTest {
	...
	//서비스에 연결하는 메서드 (제공받은 API_KEY와 URL을 이용해서 서비스에 연결한다.)
	...
	public void executeService() {
		...
		try {
			...
			//서비스의 옵션 설정
			RecognizeOptions options = new RecognizeOptions.Builder().audio(fis)	//처리할 Audio 파일 지정
