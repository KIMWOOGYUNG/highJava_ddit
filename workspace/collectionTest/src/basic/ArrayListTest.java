...
public class ArrayListTest {
	...
	public static void main(String[] args) {
		/*
		 ArrayList => 내부에서 데이터를 관리할때 배열을 이용한다.
		...
		//get()메서드로 데이터를 꺼내온다.
		System.out.println("1번째 인덱스의 자료 : " + list1.get(1));
		
		//데이터 끼워넣기도 똑같다.
		list1.add(0,"zzz");
		System.out.println("끼워넣기 후 list1 => " + list1);
		
		//데이터 수정하기 ==> set() 메서드
		String temp = (String)list1.set(0, "yyy");
		System.out.println("바뀌기 전 데이터 => " + temp);
		...
		for(int i = 0; i < list2.size(); i++){
			System.out.println(i + "번 인덱스 : " + list2.get(i));
		}
		System.out.println("----------------------------------------------");
		...
		for(int i = 0; i < strArr.length; i++){
			...
			String tempData = (String)strArr[i]; //Object형이라 String으로 형변환 필요(...?)
