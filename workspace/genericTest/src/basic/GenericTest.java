...
/*
 ...
 형식) class 클래스명 <제네릭타입글자>{
 		...
 		변환값자료형 메서드명(제네릭타입글자 변수명, ...){	//메서드의 매개변수로 사용할 경우
 			...
 		}
 	 }
...
public class GenericTest {
	...
	public static void main(String[] args) {
		...
		String temp = (String)nc1.getVal(); //조상 데이터를 자손에 넣어줄때 자료형이 다르다면 형변환 필요
		System.out.println("temp : " + temp);
		
		int intTemp = (int)nc2.getVal();
		System.out.println("intTemp : " + intTemp);
		
//		int test = (int)nc1.getVal(); //조상 클래스의 Object를 자손클래스의 int형으로 변환하는데는 문제가 없지만 사실 String형이라 오류
