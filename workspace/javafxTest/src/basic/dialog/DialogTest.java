...
public class DialogTest extends Application {
	...
	public void start(Stage primaryStage) {
		...
		btnOpen.setOnAction(e -> {
			...
			//창이 열렸을 때 보여줄 폴더(디렉토리) 초기값 설정 (지정안하면 내PC)
		...
		btnPop.setOnAction(e -> {
			...
			imgView.setImage(new Image(DialogTest.class.getResourceAsStream("../../images/ok.png")));
		...
		btnCustom.setOnAction(e -> {
			try {
				//창 스타일
				//Stage dialog = new Stage(StageStyle.DECORATED);	//창테두리
				//Stage dialog = new Stage(StageStyle.UNDECORATED);
				//Stage dialog = new Stage(StageStyle.UTILITY);
				//Stage dialog = new Stage(StageStyle.UNIFIED);
				Stage dialog = new Stage(StageStyle.TRANSPARENT);	//투명처리1
				...
				childRoot.setStyle("-fx-background-color:transparent;"); 	//투명처리2
				...
				childScene.setFill(Color.TRANSPARENT);   //투명처리3
